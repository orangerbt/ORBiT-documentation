...
\begin{table}[H]
	...
	\resizebox{0.8\textwidth}{!}{%
		...
		- & Jinzhi Cai & Add detail & 2019-7-15\\
		\end{tabu}
	}
...
This plan will use the solution that company provide and add addtional chip for the data translation from the USB to backplane bus. Because of the solution mostly will be provided by the company, it will be more stable. However, it lose the ability for upgrade.
\begin{figure}[htp]
\begin{center}
...
This plan use DSP chip as the encoder. However the dsp do not have any video interface, it will need FPGA to finish the formate translation and feed it into the DSP chip.
\begin{figure}[htp]
\begin{center}
...
\begin{table}[H]
	\centering
		\begin{tabu}{c | c }
		Pro & Con \\ \hline
		Allow futrue upgrade & Need extra chip for translation and encoding\\
		Do not need Encoding Core & Hard in PCB design(100$\Omega$ impedance) \\
